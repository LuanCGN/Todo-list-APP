...
const TodoSceen = () => {
    ...
    useEffect(() => {
        // Carregar as tarefas armazenadas ao iniciar o aplicativo
        loadTodoList();
    }, []);

    useEffect(() => {
        // Salvar a lista de tarefas sempre que ela for modificada
        saveTodoList();
    }, [todoList]);
    ...
    const renderTodos = ({ item, index }) => {
        return (
            <View style={{ backgroundColor: '#40e0d0', borderRadius: 6, paddingHorizontal: 6, paddingVertical: 5, marginBottom: 12, flexDirection: 'row', alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.8 }}>
    ...
    return (
        <View style={{ marginHorizontal: 16, paddingTop: 40 }}>
            <View style={{ flexDirection: 'row', marginBottom: 20, justifyContent: 'center', alignItems: 'center' }}>
                ...
                <Text style={{ fontSize: 30, fontWeight: 'bold', color: '#40e0d0' }}>Pixel</Text>
            </View>
            <TextInput style={{ borderWidth: 3, borderColor: '#40e0d0', borderRadius: 10, paddingVertical: 8, paddingHorizontal: 16 }} placeholder="Adicione uma Tarefa" value={todo} onChangeText={(userText) => setTodo(userText)} />
            {editedTodo ? (
                <TouchableOpacity
                    style={{ backgroundColor: '#000', borderRadius: 6, paddingVertical: 10, marginVertical: 34, alignItems: 'center' }}
